...
def calc_rint(
    ...
    return pd.concat(
        [_calc_rint_helper(measurement, file, pulse_edge_lines, pulses) for file, measurement in measurements.items()],
        ignore_index=True,
    )
...
def build_rint_lut(
    ...
    for_lut = result.loc[result["Line"].isin(valid_lines)].dropna(subset=["SOC", "Rint[Ohm]"]).copy()
    ...
    for (_file, _line), group in for_lut.groupby(["File", "Line"]):
        ...
        group = group.groupby("SOC", as_index=False)["Rint[Ohm]"].mean().sort_values("SOC")
        ...
        curve = [interp_clamped(soc, socs, rints) for soc in soc_grid]
...
def plot_lut(result: pd.DataFrame, lut: pd.DataFrame) -> None:
